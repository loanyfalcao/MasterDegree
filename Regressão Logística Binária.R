...
#mod <- glm(Gravidade ~ PosicaoVitima + Sexo + Faixa_Etaria + CondicaoPista + PerfilPista + Estacao + Período + TipoAcidente + TracadoPista + TipoLocal + Motocicleta + Veiculo_Leve + Veiculo_Pesado + Veiculo_Passageiro + Marginal + Ano_2014 + Volume_Log + Num_Veiculos + Cluster + TracadoPista*CondicaoPista,
...
# Calcule o p-valor associado à estatística de teste de Wald
p_value <- 1 - pchisq(wald_stat, df)

# Exiba a estatística de teste de Wald e o p-valor associado
...
elasticidade <- function(mod, dados) {
  # Fazendo as previsões com o modelo
  previsoes <- predict(mod, newdata = dados, type = "response")
  
  # Calculando a elasticidade
  elasticidade <- coef(mod)["Volume_Log"] * (1 - previsoes) * previsoes
  
  return(elasticidade)
}

elasticidade_volume <- elasticidade(mod, dados)

print(elasticidade_volume)

# Plotando a relação entre o volume (em logaritmo) e a elasticidade
plot(dados$Volume_Log, elasticidade_volume, 
     xlab = "Volume (em logaritmo)", ylab = "Elasticidade",
     main = "Elasticidade do Volume em relação à Gravidade")




mod2 <- glm(Gravidade ~ Sexo + Faixa_Etaria + CondicaoPista + PerfilPista + Período + TipoAcidente + TracadoPista,
            family = binomial(link = 'logit'), data = dados)
